...
class InterfazBuscaminas:
  ...
  def crear_interfaz(self):
    ...
    tk.Button(self.control_frame, text="Guardar Estado", command=self.guardar_estado).pack(side=tk.LEFT, padx=5)
    # Botón para cargar estado
    tk.Button(self.control_frame, text="Cargar Estado", command=self.cargar_estado).pack(side=tk.LEFT, padx=5)
    # Botón para reiniciar juego
    tk.Button(self.control_frame, text="Reiniciar", command=self.reiniciar_juego).pack(side=tk.LEFT, padx=5)
    ...
    for i in range(self.estado_juego['filas']):
      ...
      for j in range(self.estado_juego['columnas']):
        btn = tk.Button(self.tablero_frame, text="", width=3, height=1, font=("Courier", 12, "bold"),
                        command=lambda i=i, j=j: self.manejar_clic_izquierdo(i, j))
        btn.bind("<Button-3>", lambda event, i=i, j=j: self.manejar_clic_derecho(i, j))
  ...
  def actualizar_interfaz(self):
    ...
    for i in range(self.estado_juego['filas']):
      for j in range(self.estado_juego['columnas']):
        ...
        self.botones[i][j].config(text=texto, fg=fg, bg=bg, state="normal",
  ...
  def guardar_estado(self):
    ...
    try:
      if self.estado_juego['juego_terminado']:
        messagebox.showwarning("Juego Terminado", "No se puede guardar un juego terminado.")
        return
