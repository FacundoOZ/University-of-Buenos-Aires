...
from Buscaminas import (os,
    ...
    obtener_minas_tablero, buscar_camino, pertenece_lista,                                                                   # Ej. 6 Aux
    son_iguales_tableros,                                                                                                    # Ej. 8 Aux
    guardar_tablero, guardar_tablero_visible,                                                                                # Ej. 9 Aux
    linea_valida, contar_columnas, armar_linea, cantidad_minas_adyacentes, tablero_valido_minas, actualiza_estado,           # Ej. 10 Aux
...
class test_descubrir_celda(unittest.TestCase):
  def test_descubrir_celda_juego_terminado (self):                        # si el juego ya está terminado, no se modifica el tablero visible.
    estado: EstadoJuego = {'filas': 2,'columnas': 2,'minas': 1,
                           'tablero': [[0,-1],[1,1]],'tablero_visible': [[VACIO,VACIO],[VACIO,VACIO]],
  ...
  def test_descubrir_celda_clickear_mina (self): # si seleccionamos una mina, el juego se debería terminar y deberían mostrarse todas las minas
    estado: EstadoJuego = {'filas': 3,'columnas': 2,'minas': 2,
                           'tablero': [[1,-1],[0,1],[-1,1]],'tablero_visible': [[VACIO,VACIO],[VACIO,VACIO],[VACIO,VACIO]],
  ...
  def test_descubrir_celda_click_celda_valida (self):                     # si se descubre una celda válida, se muestra esa celda
    estado: EstadoJuego = {'filas': 2,'columnas': 2,'minas': 1,
                           'tablero': [[1,-1],[1, 1]],'tablero_visible': [[VACIO,VACIO],[VACIO,VACIO]],
                           'juego_terminado': False}
    descubrir_celda(estado, 0, 0)
    self.assertEqual(estado['tablero_visible'][0][0], '1')
    self.assertFalse(estado['juego_terminado'])
  def test_descubrir_celda_click_celda_0_con_expansion (self): # si celda es '0', verifico que se expanda bien con 'caminos_descubiertos'
    estado: EstadoJuego = {'filas': 3,'columnas': 3,'minas': 2,'tablero': [[0,0,1],[0,1,-1],[1,1,-1]],
                           'tablero_visible': [[VACIO,VACIO,VACIO],[VACIO,VACIO,VACIO],[VACIO,VACIO,VACIO]],
                           'juego_terminado': False}
    descubrir_celda(estado, 0, 0)
  ...
  def test_descubrir_celda_banderas (self):                               # este test verifica que no se revelen las banderas
    estado: EstadoJuego = {'filas': 3,'columnas': 3,'minas': 1,'tablero': [[0,1,-1],[0,1,1],[0,0,0]],
...
class test_caminos_descubiertos(unittest.TestCase):
  def test_caminos_descubiertos_celda_con_valor_mayor_a_cero(self):       # si el valor de celda es > 0, se retorna un camino de un elemento
...
class test_buscar_camino(unittest.TestCase):
  def test_buscar_camino_visitados(self):
    ...
    buscar_camino(tablero, visible, 0, 1, camino_actual, caminos, visitados) # buscar_camino sobre una celda ya visitada
    self.assertEqual(camino_actual, [])                                      # No debe modificar los caminos, porque ya fue visitada
...
class test_verificar_victoria(unittest.TestCase):
  def test_verificar_victoria_ejemplo(self): # debe dar True, pues el la única celda con mina está sin descubrir, y las otras descubiertas
    estado: EstadoJuego = {'filas': 2,'columnas': 2,'minas': 1,
  ...
  def test_verificar_victoria_tablero_sin_minas_con_todas_celdas_descubiertas(self): # no hay minas y celdas descubiertas => juego terminado
  ...
  def test_verificar_victoria_sin_minas_y_sin_celdas_descubiertas(self):           # no hay celdas reveladas, juego no terminado
    ...
    self.assertFalse(verificar_victoria (estado))
  def test_verificar_victoria_con_celdas_seguras_ocultas(self):                    # hay minas y celdas sin descubrir, juego no terminado
    estado: EstadoJuego = {'filas': 2,'columnas': 2,'minas': 1,
...
class test_reiniciar_juego(unittest.TestCase):
  ...
  def test_reiniciar_juego_reinicio_tablero_1(self): # se reinicia un tablero normal
    estado: EstadoJuego = {'filas': 3,'columnas': 3,'minas': 2,
                           'tablero': [[0,1,-1],[1,2,1],[0,1,0]],'tablero_visible': [['0','1',VACIO],['1','2','1'],[VACIO,'1','0']],
                           'juego_terminado': True}
    reiniciar_juego(estado)
  ...
  def test_reiniciar_juego_reinicio_tablero_2(self):
    estado: EstadoJuego = {'filas': 4,'columnas': 4,'minas': 3,
                           'tablero': [[0,1,-1,1],[1,2,2,1],[1,-1,2,1],[1,1,1,0]],
                           'tablero_visible': [[VACIO,'1',VACIO,'1'],['1','2','2','1'],['1',VACIO,'2','1'],['1','1','1',VACIO]],
                           'juego_terminado': True}
    reiniciar_juego(estado)
...
# Ejercicio 9 #
#————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
"""Nota: la consigna pide "analizar obligatoriamente casos de test para la función que pide el enunciado". Debido a que nuestra función NO
tiene condicionales, pero si otras funciones auxiliares dentro, esto implica que no podemos testear la función original (no hay ramas, o mejor
dicho, las ramas están dentro de las funciones aux.), pero entonces ==> DEBEMOS testear nuestras funciones auxiliares. Por lo tanto, solo
testeamos nuestras funciones:"""
class test_guardar_estado(unittest.TestCase):
  ...
  def test_guardar_estado_funcion_guardar_tablero_3x3(self):      # Función Aux 1
    estado: EstadoJuego = {'filas': 3,'columnas': 3,'minas': 2,'tablero': [[-1,1,0],[1,2,1],[0,1,-1]],
                           'tablero_visible': [[VACIO,VACIO,VACIO],[VACIO,VACIO,VACIO],[VACIO,VACIO,VACIO]],
                           'juego_terminado': False}
  ...
  def test_guardar_estado_funcion_guardar_tablero_visible_3x3(self):              # Función Aux 2
    estado: EstadoJuego = {'filas': 3,'columnas': 3,'minas': 2,'tablero': [[2,-1,2],[2,-1,2],[1,1,1]],
                           'tablero_visible': [[VACIO,BANDERA,'2'],['2',VACIO,'2'],[VACIO,'1',VACIO]],
                           'juego_terminado': False}
...
class test_estado_valido(unittest.TestCase):
  ...
  def test_estado_valido_elem_visible_no_coincide_con_valor_real(self):
    estado = {'filas': 2, 'columnas': 2, 'minas': 1, 'tablero': [[0, 1], [1, -1]],
              'tablero_visible': [['5', VACIO], [VACIO, VACIO]],  # '5' no coincide con el valor real 0
              'juego_terminado': False}
    self.assertFalse(estado_valido(estado))
class TestActualizaEstado(unittest.TestCase): # Agregamos un test para la función auxiliar actualiza_estado
  def test_actualiza_estado(self):
    ...
    minas = 1
    columnas = 2
    actualiza_estado(estado, tablero, visible, minas, columnas)
    self.assertEqual(estado["filas"], 2)
    self.assertEqual(estado["columnas"], 2)
    self.assertEqual(estado["minas"], 1)
    self.assertEqual(estado["tablero"], [[-1, 1], [1, 1]])
    self.assertEqual(estado["tablero_visible"], [[" ", "1"], [" ", " "]])
    self.assertFalse(estado["juego_terminado"])
#————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
#————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
