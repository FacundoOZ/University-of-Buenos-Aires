...
public class ABB<T extends Comparable<T>> {
  ...
  // Atributos Privados:
  private Nodo _raiz;       // "_raiz" es el Nodo de arriba de todo,
  ...
  //==========================================================================================================================================
  // Handle del Árbol Binario de Búsqueda (ABB)
  //==========================================================================================================================================

  //——————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
  // Ejercicio 1 //
  //——————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
  /*Definir la clase HandleABB que implemente la interfaz Handle. Esta clase debe contener una referencia al nodo del árbol binario de
  búsqueda que contiene el elemento referenciado por el handle.*/
  public class HandleABB implements Handle<T> {
    // Creo los atributos privados:
    private Nodo _elemento;     // 'elemento' represental el 'Nodo' del ABB al cual el Handle apunta en el ABB.
    private HandleABB(Nodo n) { // Creo el constructor Handle dentro de la clase del HandleABB
      this._elemento = n;       // y su '_elemento' serán los pasados por parámetro (arbol y n)
    }
  
    //————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
    // Ejercicio 2 // Implementar el método void valor() para obtener el valor del nodo referenciado por el handle.
    //————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
    public T valor() {
      return _elemento.valor; // El valor del HandleABB, será simplemente el valor del nodo al que apunta (el Nodo _elemento).
    }

    //————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
    // Ejercicio 3 // Implementar el método void eliminar() para eliminar el nodo referenciado por el handle.
    //————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
    public void eliminar(){
      if (_elemento == null) return;                            // CASO 0: El elemento no existe (el Handle no referencia a nadie).
      Nodo j = _elemento;                                       // Si no, llamo j al nodo del Handle: "_elemento", y evaluo sus vecinos:
      ...
      _elemento = null;                                         // Una vez terminado los casos, remuevo el Nodo "_elemento" del Handle.
    }
  }

  //==========================================================================================================================================
  // Árbol Binario de Búsqueda (reutilizamos completamente el ABB del Taller 4):
  //==========================================================================================================================================

  //——————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
  // Ejercicio 4 // Modificar el método void insertar(T elem) para que devuelva un handle al agregar un elemento al árbol.
  //——————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
  public HandleABB insertar(T elem) {
    if (_raiz == null) {                             // Si el ABB es vacío,
      _raiz = new Nodo(elem);                        // creamos un nodo con el elemento "elem" de tipo T en la "_raíz".
      _cardinal = 1;                                 // -> cardinal será = 1,
      return new HandleABB(_raiz);                   // y creamos el Handle cuyo Nodo _elemento = _raiz.
    }
    Nodo j = _raiz;                                  // Si no, creo un Nodo aux que representa un nodo j-ésimo (arranca en _raiz).
    while (true) {                                   // Chequear siempre:
      if (elem.compareTo(j.valor) == 0) return null; // Si el nodo j actual = elem -> ya existe -> no hago nada -> return.
      if (elem.compareTo(j.valor) > 0) {             // Si elem > j.valor -> debo ir al sub-árbol derecho:
        if (j.der == null) {                         // Si no hay nadie a mi derecha -> listo!
          ...
          _cardinal++;                               // -> incremento _cardinal en 1,
          return new HandleABB(j.der);               // y devuelve el Handle cuyo Nodo _elemento = hijo derecho del Nodo j.
        }
        ...
        if (j.izq == null) {                         // Si no hay nadie a mi izquierda -> listo!
          ...
          _cardinal++;                               // -> incremento _cardinal en 1,
          return new HandleABB(j.izq);               // y devuelve el Handle cuyo Nodo _elemento = hijo izquierdo del Nodo j.
        }
  ...
  //==========================================================================================================================================
  // Iterador del Árbol Binario de Búsqueda (ABB)
  //==========================================================================================================================================
  public class ABB_Iterador {
    
    // Atributos Privados:
    private Nodo _iter, _prox;  // los nodos _iter (un nodo i-ésimo) y _prox (el (i+1)-ésimo) (su siguiente).
